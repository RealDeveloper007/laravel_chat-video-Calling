...
if(location.protocol === 'http:'){
    wsChat = new WebSocket("ws://domain.it/comm");
}

else{
    wsChat = new WebSocket("wss://domain.it/wss/comm");
}
...
function myStopFunction(start) 
{
    if(start=='yes')
    {
        console.log('start');
        var CounterInterval = setInterval(startCounter, 1000);
...
window.addEventListener('load', function(){
        ...
        if(data.room === room){
            ...
            switch(data.action){
                case 'initCall':
                    ...
                    document.getElementById('calleeInfo').innerHTML = 'Video Call from '+Remoteusername;
                ...
                case 'callRejected':
                    ...
                    document.getElementById("callerInfo").innerHTML = 'Video Call to '+Remoteusername;
                ...
                case 'endCall':
                    ...
                    document.getElementById("calleeInfo").innerHTML = 'Video Call to '+Remoteusername;
                ...
                case 'typingStatus':
                    if(data.status){
                        document.getElementById("typingInfo").innerHTML = Remoteusername+" is typing";
                    }
                ...
                case 'newSub':
                    ...
                    showSnackBar(Remoteusername+" entered room", 10000);
                ...
                case 'imOffline':
                    ...
                    showSnackBar(Remoteusername+" left room", 10000);
                    enableCallBtns();
                    break;
            }  
        }
    ...
    */
    ...
    document.getElementById("rejectCall").addEventListener('click', function(e){
        ...
        wsChat.send(JSON.stringify({
            action: 'callRejected',
            msg: "Call rejected by "+Remoteusername,
            room: room
        }));
    ...
    */
    ...
    document.getElementById("endCall").addEventListener('click', function(e){
        ...
        endCall("Call ended by "+Remoteusername, false);
...
function initCall()
{
    var ConnectStatus = $('#remoteStatusTxt').text();
    ...
    if(checkUserMediaSupport){
        ...
        callerInfo.innerHTML = callType === 'Video' ? 'Video call to '+Remoteusername : 'Audio call to Remote';
        ...
        wsChat.send(JSON.stringify({
            action: 'initCall',
            msg: callType === 'Video' ? "Video call from "+Remoteusername : "Audio call from remote",
            room: room
        }));
    ...
    document.getElementById("callModal").style.display = 'block';
     ...
     showSnackBar("Sorry! "+Remoteusername+" is currently Offline. Please wait until Online.", 15000);
        ...
        myPC.onsignalingstatechange = function(){
            switch(myPC.signalingState){
                case 'closed':
                    ...
                    swal({title: "Oops!", text: "Call is disconnected ", type: "error"},
                                    function(){ 
                                      StoreVideoDetails();
                                       location.reload();
                                    }
...
function handleCallTermination(){
    ...
    showSnackBar("Call terminated by "+Remoteusername, 10000);
...
function RemainTimer(){
    var timeleft = minute_balance;
    var downloadTimer = setInterval(function(){
      document.getElementById("progressBar").value = minute_balance - timeleft;
      timeleft -= 1;
      if(timeleft <= 0){
        
                    swal({title: "Sorry!", text: "Your Token balance is too low.Please buy tokens ", type: "error"},
                                    function(){ 
                                      window.href.location="https://www.google.com";
                                    }
                                 );
